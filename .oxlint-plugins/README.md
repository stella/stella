...
- [`scanned-file-boundary`](./scanned-file-boundary.ts) (`scanned-file-boundary`): rejects casts to `ScannedFile` or `FileKey`, prototype-built `ScannedFile`s, restricted mint imports, and direct folio-core DOCX parser imports or `FolioDocxReviewer.fromBuffer` calls outside their owning modules, so parser inputs are only ever scanned uploads or bytes read back from a file key.
