...
- [`no-native-s3-object-read`](./no-native-s3-object-read.ts) (`no-native-s3-object-read`): confines object reads to the owned storage boundary, which selects native or presigned-fetch transport according to cancellation, bounds, and response-validation requirements.
