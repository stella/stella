...
- [`scanned-file-boundary`](./scanned-file-boundary.ts) (`scanned-file-boundary`): rejects casts to `ScannedFile` or `FileKey`, prototype-built `ScannedFile`s, restricted mint, derived-file, and stored-row imports, and direct folio-core DOCX parser imports or `FolioDocxReviewer.fromBuffer` calls outside their owning modules, so parser inputs (the template and style-set parsers included) are scanned uploads, bytes read back from a file key or a row recorded as scanned, case-law publisher downloads, or a server rewrite of one of those.
