...
- [`forbid-process-env-outside-env-ts`](./forbid-process-env-outside-env-ts.ts) (`forbid-process-env-outside-env-ts`, `runtime-mode-keys`): confines unvalidated environment reads (`process.env` in any spelling, `node:process` imports, `Bun.env`, and server-side `import.meta.env`) to env modules, config and test files, repository- or package-root `scripts/` and `tests/` directories, and explicitly configured files and directories; `runtime-mode-keys` confines `NODE_ENV` and `STELLA_LOCAL_DEV`, including reads through an alias of the environment object, to `@stll/runtime-mode`.
