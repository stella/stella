...
- `no-layout-motion-classes` rewrites `transition-all` to `transition` and the
  `screen` viewport utilities to their `dvh`/`dvw` equivalents in provable class
  strings.
...
- [`no-layout-motion-classes`](./no-layout-motion-classes.ts) (`no-layout-motion-classes`): guards the motion and viewport conventions over Tailwind class strings. Rejects `transition-all`, a `transition-` or `animate-[…]` utility naming a layout property (`width`, `height`, `top`, `left`, `right`, `bottom`, `inset`, `margin`, `padding`), and the `h-screen`/`min-h-screen`/`max-h-screen`/`w-screen` viewport utilities, which resolve against `100vh`/`100vw` and ignore mobile browser chrome. Every class string in scope is checked, so a utility in a `cn()`/`cva()`/`clsx()` argument or a variant map is reported like one written on the element; the autofix is confined to a direct `className` value or a direct string argument of a class composer. A surface that changes its own box (a disclosure panel, a collapsible rail, a popup positioner repositioning on collision, a progress bar) has no compositable spelling, so it is named in `allowedFiles` as `{ path, reason }`; the reason names the surface that owns the box, and the exemption covers the layout-property diagnostics only, leaving `transition-all` and the viewport utilities reported in those files too.
