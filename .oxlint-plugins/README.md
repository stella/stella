...
- [`scanned-file-boundary`](./scanned-file-boundary.ts) (`scanned-file-boundary`): rejects casts to `ScannedFile` or `FileKey` and prototype-built `ScannedFile`s outside their owning modules, so parser inputs are only ever scanned uploads or bytes read back from a file key.
