...
- [`no-native-s3-object-read`](./no-native-s3-object-read.ts) (`no-native-s3-object-read`): routes object reads through the compatibility wrapper while Stella is pinned to Bun 1.3.14; remove the rule after the runtime upgrade removes that boundary.
