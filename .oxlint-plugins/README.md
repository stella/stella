...
- [`no-unreviewed-typebox-unsafe`](./no-unreviewed-typebox-unsafe.ts) (`no-unreviewed-typebox-unsafe`): rejects TypeBox/Elysia `Unsafe` calls outside exact reviewed adapter bindings; tracks imported aliases and permits local shadowing.
