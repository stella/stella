...
- [`no-literal-minor-unit-scale`](./no-literal-minor-unit-scale.ts) (`no-literal-minor-unit-scale`): rejects scaling a money-named value by a literal 100, which hard-codes an exponent the currency owns (JPY has none, KWD has three); convert with `toMinorUnits`/`toMajorUnits` from `@stll/money`. Percent arithmetic against a 100 base is unaffected.
