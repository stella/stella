...
- [`no-custom-account-modal`](./no-custom-account-modal.ts) (`no-custom-account-modal`): keeps one modal asking a reader for an account, `SignInDialog`. Reports, at its modal import, a web module that imports a dialog, alert dialog or sheet and also reaches account entry: the public shell's sign-in hand-off (`usePublicSignInRequest`, `PublicSignInRequestContext`), an account-entry message key (`auth.signIn`, `auth.createFreeAccount`, the provider buttons), `authClient.signIn`/`signUp`, or a JSX `to="/auth"`. A gated act calls `useRequireAccount()` instead, which opens the canonical dialog directly. Other `auth.*` keys and loader redirects do not count, so settings dialogs and the protected layout stay clean; `sign-in-dialog.tsx` is the only owner.
