...
- [`no-custom-account-modal`](./no-custom-account-modal.ts) (`no-custom-account-modal`): keeps one modal asking a reader for an account, `SignInDialog`. Reports, at its modal import, a web module that renders a dialog, alert dialog or sheet and reaches account entry from that modal's code: the public shell's sign-in hand-off (`usePublicSignInRequest`, `PublicSignInRequestContext`), an account-entry message key (`auth.signIn`, `auth.createFreeAccount`, the provider buttons), `authClient.signIn`/`signUp`, or a JSX `to="/auth"` in any static spelling. The modal's code is the top-level declaration rendering it, the same-module declarations it references, and those that render it; an unrelated sign-in link elsewhere in the module does not count. A gated act calls `useRequireAccount()` instead, which opens the canonical dialog directly. Other `auth.*` keys and loader redirects do not count, so settings dialogs and the protected layout stay clean; `sign-in-dialog.tsx` is the only owner. Not caught: composition across modules, aliases and computed keys, generic copy around `useRequireAccount()`, and a hand-off in a helper that only the modal's renderer references; one declaration rendering an ordinary dialog beside a sign-in link is still reported.
