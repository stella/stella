...
- [`decision-shaped-output-schema`](./decision-shaped-output-schema.ts) (`decision-shaped-output-schema`): reports a `generateTanStackObjectForRole` / `streamTanStackObjectForRole` call whose `outputSchema` resolves, in the same file, to a valibot object literal with no free text in it: every entry a picklist, literal, boolean, number, ISO date, or an array/object/union/optional of those. That output is a decision, so it belongs to `decide()` / `decideMany()` in `apps/api/src/lib/decisions/decide.ts`, which prices it as one and logs the reading. One `v.string()` that no `isoDate`/`isoDateTime` action pins keeps the call generative. Resolution is inline or through a same-file `const`, with one level of `v.pipe(schema, ...)` peeled; a schema from another module, a spread entry list, or an unrecognized valibot call is left alone.
