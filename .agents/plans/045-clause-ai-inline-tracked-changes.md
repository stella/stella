...
- **Skip Folio's DOCX-coupled apply engine.** `applyFolioAIEditOperations` and
  ...
  Instead, apply the AI's revised paragraphs into the clause doc *with
  suggestionMode active*, so ordinary replace/insert/delete transactions are
...
- Tracked-change persistence across save: accepted text is saved; the design
  question of saving *pending* marks is deferred (see Open Questions).
...
- **Marks are a thin TipTap shim, not Folio's marks.** Folio's
  ...
  `diffWordSegments` — is reused from `@stll/folio` (newly exported).
...
- **No add/remove paragraph handling needed.** `rewrite.ts` returns an
  ...
  same index.
- **i18n reuses existing keys** (`common.accept`, `docxReview.{reject,acceptAll,rejectAll}`)
  rather than adding duplicates that the common-overlap gate would flag; one new
  key `clauses.aiNoChanges`.
