...
const READER_ROLE = stellaCaseLawAnalysisReader.name;
...
const expectedSelectColumns = Object.entries(
  CASE_LAW_ANALYSIS_READER_SELECT_COLUMNS,
)
...
describe("case-law analysis reader role", () => {
  ...
  test("SET ROLE is refused columns outside the grant and every write", async () => {
    for (const statement of [
      `SELECT "fulltext" FROM "case_law_decisions"`,
      `SELECT "source_id" FROM "case_law_decisions"`,
      `SELECT "citation_key" FROM "case_law_citations"`,
      `SELECT "confidence" FROM "case_law_polarity_rules"`,
      `SELECT "weight" FROM "case_law_court_weights"`,
      ...
      `DELETE FROM "case_law_polarity_rules"`,
      `INSERT INTO "case_law_court_weights" ("country") VALUES ('CZE')`,
...
describe("case-law analysis reader migrations", () => {
  test("effective migration grants equal the source-of-truth map", () => {
    ...
    expect(sortedRecord(foldReaderGrants(sources))).toEqual(
      sortedRecord(Object.entries(CASE_LAW_ANALYSIS_READER_SELECT_COLUMNS)),
    );
  });

  test("rejects reader grant syntax outside the audited grammar", () => {
    for (const statement of [
      'GRANT SELECT ON TABLE "case_law_decisions" TO stella_case_law_analysis_reader;',
