  ...
  const txResultResult = await safeDb(async (tx) => {
    ...
    if (deleteSource) {
      ...
      for (const id of sourceEntityIds.toReversed()) {
        // oxlint-disable-next-line no-db-await-in-loop/no-db-await-in-loop, no-await-in-loop, react-doctor/async-await-in-loop -- sequential deletes in reverse (children-first) order respect FK constraints within the transaction
