...
const buildActiveDocxEditPrompt = (activeFile: IncomingActiveFile) => {
  ...
  return [
    ...
    "CITATIONS IN PLAIN ANSWERS: When you summarise, quote, or refer to specific content from the open document in a normal text reply (i.e. NOT inside `apply-active-docx-edits`), wrap the supporting paragraph snippet in a Markdown link whose href is `folio:<blockId>`. Example: `the contract is governed by [Czech law](folio:b-0042)`. The link text should be a short quote or paraphrase of the cited block; the href is the matching `blockId` from the JSON list below. The UI renders these as click-to-scroll citation chips that jump the editor to the block. Cite at most a few blocks per reply (only the ones a user would want to verify); never invent a blockId that's not in the list.",
    truncationNotice,
