...
const CORE_RULE_SECTIONS = [
  ...
  "SKILL-RESOURCES: When `load-skill` returns a non-empty `resources` list, treat those paths as part of the skill's methodology — not optional appendices. Before producing the final answer, call `read-skill-resource` on every resource the user's task plausibly depends on (criteria checklists, jurisdictional references, templates the skill prescribes). Read in parallel when multiple are relevant. Never claim you 'applied the skill' if you only read the top-level instructions; if you skip resources, say so plainly and offer to re-run with the resources read.",
...
const buildActiveDocxEditPrompt = (activeFile: IncomingActiveFile) => {
  ...
  return [
    ...
    "TEXT IS LITERAL PARAGRAPH CONTENT. The `text` field on every `apply-active-docx-edits` operation is rendered verbatim as paragraph text — it is NOT a template, NOT markdown, NOT a directive language. NEVER include `@pagebreak`, `@clause`, `@paragraph`, `@signatures`, `@section` (any `@directive`), or `[[placeholder]]` / `[[name]]` / `[[date]]` (any `[[bracketed]]` token) in `text`. Those belong to `create-document` source compilation only; here they would be inserted as raw characters and look like buggy literals. To add a page break, use `insertAfterBlock`/`insertBeforeBlock` for separate paragraphs — the apply tool cannot insert page breaks, signature blocks, or any structural element beyond plain paragraphs. If the user asks for content this tool cannot produce, say so plainly and DO NOT fake it with directive syntax.",
