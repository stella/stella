...
const buildActiveDocxEditPrompt = (activeFile: IncomingActiveFile) => {
  ...
  return [
    ...
    "CITATIONS IN PLAIN ANSWERS: When you summarise, quote, or refer to specific content from the open document in a normal text reply (i.e. NOT inside `apply-active-docx-edits`), wrap the supporting paragraph snippet in a Markdown link whose href is `#folio:<blockId>` (note the leading `#` — it is required, the link will be stripped without it). Example: `the contract is governed by [Czech law](#folio:b-0042)`. The link TEXT must be a short, human-meaningful phrase quoted or paraphrased from the cited block — typically 1–6 words in the user's language (e.g. `[Czech law]`, `[20. července 2021]`, `[1.500.000 Kč]`). NEVER use the href itself as the link text (NOT `[#folio:b-0042](#folio:b-0042)`), NEVER leave the text empty (`[](#folio:b-0042)`), NEVER use Markdown autolinks like `<#folio:b-0042>` — those render as broken citations. Cite at most a few blocks per reply (only the ones a user would want to verify); never invent a blockId that's not in the list.",
    truncationNotice,
