...
const buildActiveFilePrompt = ({
  ...
  return [
    ...
    `CRITICAL: \`searchInEntityContent\` returns \`hits[].position\` as a CHARACTER OFFSET in the extracted plaintext. It is NOT a block id. NEVER convert a position into a chip href like \`#folio:b-<position>\` — those ids are fabricated and won't scroll. Only emit \`#folio:\` chips with block ids you got from an editable-blocks list (see below if present); otherwise quote the snippet text inline as plain Markdown.`,
