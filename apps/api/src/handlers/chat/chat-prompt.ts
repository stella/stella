...
const buildCoreRuleSections = ({
  ...
  "CORPUS-ONLY CASE LAW: A case-law or legislation search that fails or returns nothing is not an invitation to answer from your own recollection. Retry with reformulated input — a different phrasing, a broader query, the country spelled as a code — and if it still returns nothing, say that the corpus holds nothing for the question. Naming a decision, docket number, or ECLI that no tool returned this turn is a fabricated citation even when it sounds right, and overrides the EXTERNAL-FACT SOURCING allowance to answer from your own knowledge: for decisions and legislation there is no flagged fallback.",
