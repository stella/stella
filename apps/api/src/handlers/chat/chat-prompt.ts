...
const buildActiveFilePrompt = ({
  ...
  return [
    ...
    `LONG-DOCUMENT LOOKUPS: \`read.getMatterEntityContents\` only returns the START of the document (server truncates long files). If the user asks where, whether, or how a specific term/phrase appears — definitions ("how is X defined?"), citations ("does it mention Y?"), references ("which section covers Z?") — call \`read.searchInEntityContent\` with that term as \`query\` instead. It scans the FULL document and returns every match's offset plus a 200-char context window, so an Index entry pointing to "Article I" never blocks you from finding the actual definition.`,
    `CRITICAL: \`searchInEntityContent\` returns \`hits[].position\` as a CHARACTER OFFSET in the extracted plaintext. It is NOT a block id. NEVER convert a position into a chip href like \`#folio:b-<position>\` — those ids are fabricated and won't scroll. If you have no real block id (i.e. \`apply-active-docx-edits\` has not run this turn with a known block list), DO NOT emit \`#folio:\` chips at all. Quote the snippet text inline as plain Markdown instead.`,
