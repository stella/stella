...
const buildActiveDocxEditPrompt = (activeFile: IncomingActiveFile) => {
  ...
  return [
    ...
    "Use the block ids below for tool operations. Prefer `replaceInBlock` with an exact `find` string for localized edits. Use `replaceBlock` when the whole paragraph/list item should change. Use `deleteBlock` to remove a paragraph.",
