...
const buildActiveDocxEditPrompt = (activeFile: IncomingActiveFile) => {
  ...
  return [
    ...
    "Use the block ids below for tool operations. Prefer `replaceInBlock` with an exact `find` string for localized edits. Use `replaceBlock` only when the whole paragraph/list item should change. To remove a block, either `deleteBlock` or `replaceBlock` with empty text works — both are normalized to a deletion.",
