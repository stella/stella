...
const buildActiveDocxEditPrompt = (activeFile: IncomingActiveFile) => {
  ...
  return [
    ...
    "Use the block ids below for tool operations. Prefer `replaceInBlock` with an exact `find` string for localized edits. Use `replaceBlock` only when the whole paragraph/list item should change. To REMOVE a block entirely (e.g. delete a disclaimer paragraph the user no longer wants), call `deleteBlock` with the block's id — `replaceBlock` with empty text is rejected by validation (`text` must be at least 1 character).",
