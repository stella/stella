...
const CONTACT_EXPORT_CSV_MEDIA_TYPE = "text/csv; charset=utf-8";
...
const config = {
  ...
  transport: {
    ...
    response: {
      mediaTypes: [
        CONTACT_EXPORT_CSV_MEDIA_TYPE,
        CONTACT_EXPORT_JSON_MEDIA_TYPE,
      ],
    },
    alternative: {
      type: "partial",
      via: ["contacts.list"],
      limitation:
        "paginates the directory as JSON records without the portable import field set",
    },
  },
...
const serializeContactExport = (
  ...
  if (format === "json") {
    return {
      ...
      contentType: CONTACT_EXPORT_JSON_MEDIA_TYPE,
  ...
  return {
    body: csvRows.join("\n"),
    contentType: CONTACT_EXPORT_CSV_MEDIA_TYPE,
