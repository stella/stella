    ...
    await Result.tryPromise({
      try: async () => {
        ...
        for (let i = 0; i < items.length; i += chunkSize) {
          ...
          // oxlint-disable-next-line no-await-in-loop, react-doctor/async-await-in-loop -- sequential chunk processing: abort/rewind bookkeeping depends on prior chunk completing
