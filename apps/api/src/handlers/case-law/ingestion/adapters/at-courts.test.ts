...
describe("at-courts adapter", () => {
  it("has correct metadata", () => {
    ...
    expect(atCourtsAdapter.minRequestIntervalMs).toBe(
      1000,
    );
    expect(atCourtsAdapter.pageTimeoutMs).toBe(
      220_000,
    );
  });

  it(
    "fetches first page with fulltext and ECLI",
    async () => {
      const result = await atCourtsAdapter.fetchPage(
        null,
        {},
      );
      ...
      const withEcli = page.decisions.find(
        (d) => d.ecli,
      );
      ...
      const withFulltext = page.decisions.find(
        (d) => d.fulltext,
      );
      expect(withFulltext).toBeDefined();
      expect(
        withFulltext?.fulltext?.length,
      ).toBeGreaterThan(100);

      // documentUrl populated from HTML fulltext link
      const withDocUrl = page.decisions.find(
        (d) => d.documentUrl,
      );
      expect(withDocUrl).toBeDefined();
      expect(withDocUrl?.documentUrl).toContain(
        "ris.bka.gv.at",
      );
    },
    120_000,
