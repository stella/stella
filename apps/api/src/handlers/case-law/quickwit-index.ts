...
import { caseLawIndexConfig } from "@/api/lib/legal-search/quickwit-index-config";

/**
 * Quickwit search-projection maintenance. Mirrors search-index.ts (the
...
// have confirmed this process so we don't probe Quickwit every batch.
...
/** Build the Quickwit search document, omitting empty optional fields. */
 ...
 */
export const backfillQuickwitIndex = async (
  scopedDb: ScopedDb,
  batchSize: number,
  generation: string,
): Promise<number> => {
...
/**
 * Remove a decision from a Quickwit generation (GDPR/takedown). Uses a
 * delete-task (Quickwit's async delete path) and records the operation.
 */
export const removeDecisionFromQuickwit = async (
  decisionId: SafeId<"caseLawDecision">,
  scopedDb: ScopedDb,
  indexId: string,
