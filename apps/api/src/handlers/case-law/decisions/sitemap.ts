...
export const listSitemapShardDecisionsHandler = async (
  ...
  const queryResult = await caseLawDb(async (tx) => {
    ...
    for (const groupKeyBatch of chunkArray(
      ...
      // oxlint-disable-next-line no-db-await-in-loop/no-db-await-in-loop, no-await-in-loop, react-doctor/async-await-in-loop -- sequential by design: sequential reads on the same transaction connection (one in-flight query per tx)
