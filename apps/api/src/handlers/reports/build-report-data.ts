...
// oxlint-disable-next-line no-restricted-imports -- brands field/entity ids returned by queryEntities (server-validated, workspace-scoped) to re-hydrate their justifications and review decisions
      ...
      const justificationRows = yield* Result.await(
        safeDb(async (tx) => {
          ...
          for (
            ...
            // oxlint-disable-next-line no-db-await-in-loop/no-db-await-in-loop -- sequential reads on one tx connection; the batch caps each `IN (...)` below the bound-parameter limit
