...
const getOverview = createSafeRootHandler(
  ...
  async function* ({ safeDb, session }) {
    ...
    const fromInstant = nowInstant
      .toZonedDateTimeISO("UTC")
      .subtract({ days: OVERVIEW_DAYS - 1 })
      .startOfDay()
      .toInstant();
    const now = new Date(nowInstant.epochMilliseconds);
    const from = new Date(fromInstant.epochMilliseconds);
