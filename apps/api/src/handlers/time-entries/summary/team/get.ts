...
const MAX_TEAM_SUMMARY_ROWS =
  LIMITS.workspaceMembersCount * MAX_SUMMARY_DAYS;

const teamTimeEntrySummaryQuerySchema = t.Object({
  dateFrom: t.String({ format: "date" }),
  dateTo: t.String({ format: "date" }),
});

const readTeamTimeEntrySummary = createSafeHandler(
  {
    description:
      "Summarize each matter member's time by day for a bounded date range.",
    permissions: { timeEntry: ["approve"] },
    mcp: { type: "capability", reason: "billing_admin" },
    access: "read",
    query: teamTimeEntrySummaryQuerySchema,
  },
  async function* ({ query, safeDb, session, user: currentUser, workspaceId }) {
...
export default readTeamTimeEntrySummary;
