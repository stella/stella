    ...
    const txResult = yield* Result.await(
      safeDb(async (tx) => {
        ...
        for (let i = 0; i < body.splits.length; i++) {
          ...
          // oxlint-disable-next-line no-db-await-in-loop/no-db-await-in-loop, no-await-in-loop, react-doctor/async-await-in-loop -- sequential inserts create ordered successor rows for the split entry
