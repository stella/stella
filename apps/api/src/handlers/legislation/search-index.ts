...
export const backfillLegislationSearchIndex = async (
  ...
  for (let i = 0; i < rows.length; i += SEARCH_INDEX_CONCURRENCY) {
    ...
    // oxlint-disable-next-line no-await-in-loop, no-db-await-in-loop/no-db-await-in-loop, react-doctor/async-await-in-loop -- bounded concurrency: each SEARCH_INDEX_CONCURRENCY chunk drains before the next so tsvector upserts don't overwhelm Postgres
