...
/**
 * A run with one candidate and deliberately NO candidate-source rows: the
 * finalizing transaction compares the live sources against the set the claim
 * captured and reports `source-missing` when there are none, which is the
 * compensation path both tests need.
 */
const seedAcceptance = async (
  ownership: SeededOwnership,
): Promise<SeededAcceptance> => {
...
const acceptCandidate = async (seeded: SeededAcceptance) =>
  await acceptGenerationCandidate.handler(
...
test("releasing an adopted reservation keeps the committed item alive", async () => {
  const seeded = await seedAcceptance("adopted");
...
test("releasing a reservation this request created deletes its entity", async () => {
  const seeded = await seedAcceptance("created-here");
