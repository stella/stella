...
export const entityDeletionCleanupRequests = p.pgTable(
  ...
  (table) => [
    ...deletionCleanupConstraints({
      table,
      prefix: "entity_deletion_cleanup",
      statusSqlValues: ENTITY_DELETION_CLEANUP_STATUS_SQL_VALUES,
    }),
...
export const entityDeletionEffectChunks = p.pgTable.withRLS(
  ...
  (table) =>
    destructiveEffectChunkConstraints({
      table,
      requestIdColumn: entityDeletionCleanupRequests.id,
      prefix: "entity_deletion_effect_chunks",
    }),
);
...
export const entityVersions = p.pgTable(
  "entity_versions",
  {
    ...
    /**
     * The document's own language as declared by DOCX `w:lang` run defaults
     * (see `lib/document-translation/docx-language.ts`), stored as a
     * `DOCUMENT_TRANSLATION_SOURCE_LANGUAGES` code: BCP-47 shaped, e.g. `CS`,
     * `EN-GB`, `PT-PT`. Native extraction writes it once. Null means the DOCX
     * has no usable declaration, predates this metadata, or is another format.
     */
    detectedLanguage: p
...
export const folioCollabRooms = p.pgTable(
  "folio_collab_rooms",
  {
    ...
    sourceVersionId: safeUuid<"entityVersion">("source_version_id").notNull(),
    fileName: p.varchar("file_name", { length: 256 }).notNull(),
  ...
  (table) => [
    ...
    p
      .index("folio_collab_rooms_workspace_source_version_idx")
      .on(table.workspaceId, table.sourceVersionId),
    p.check(
    ...
    p.foreignKey({
      columns: [table.sourceVersionId, table.entityId, table.workspaceId],
      foreignColumns: [
        entityVersions.id,
        entityVersions.entityId,
        entityVersions.workspaceId,
