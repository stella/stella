...
export const caseLawIndexJobs = p.pgTable(
  "case_law_index_jobs",
  {
    ...
    /**
     * The generation an operation targeted, where it targeted one. An erasure
     * or a withdrawal targets whichever generations hold the document, so it
     * names none.
     */
    generation: p.varchar({ length: 64 }),
    operation: p
      .varchar({ length: 16 })
      .notNull()
