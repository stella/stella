...
export const createEntityVersionFromBuffer = async ({
  ...
  // Server-generated bytes derive from the current version's file, which may
  // itself have arrived stamped. Size and hash below come from these bytes, so
  // the version records what storage actually holds.
