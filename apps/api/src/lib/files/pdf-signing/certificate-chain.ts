 ...
 * means none were sent, and it may mix in other certificate choices.
 */
export const carriedCertificates = (
  entries: readonly unknown[] | undefined,
): pkijs.Certificate[] =>
  entries === undefined
    ? []
    : entries.filter((entry) => entry instanceof pkijs.Certificate);
...
const certificatesIn = (bytes: Uint8Array): pkijs.Certificate[] => {
  ...
  return Result.try((): pkijs.Certificate[] => {
    ...
    return carriedCertificates(signedData.certificates);
  }).unwrapOr([]);
};
