...
describe("cursor pagination", () => {
  ...
  test("validates microsecond timestamp cursor parts", () => {
    ...
    expect(
      isMicrosecondTimestampPaginationCursorPart("2026-02-30T00:00:00.000000"),
    ).toBe(false);
    expect(isMicrosecondTimestampPaginationCursorPart("not-a-timestamp")).toBe(
      false,
    );
    expect(isMicrosecondTimestampPaginationCursorPart(12_345)).toBe(false);
  });
});
  ...
  test("isMicrosecondTimestampPaginationCursorPart accepts to_char-style microsecond timestamps", () => {
    fc.assert(
      fc.property(boundedDate, (d) => {
        ...
        const microsecond = d
          .toISOString()
          .replace(/\.(?<milliseconds>\d{3})Z$/u, ".$<milliseconds>000");
