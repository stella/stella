...
if (!databaseUrl || !runPostgresTests) {
  ...
  describe("scheduler lease handoff (postgres)", () => {
    test("a runner whose lease is taken stops its task; the new owner completes it once", async () => {
      const firstClient = new SQL({ url: databaseUrl, max: 2 });
      const secondClient = new SQL({ url: databaseUrl, max: 2 });
      ...
      try {
        ...
        await secondDb
          ...
          .where(eq(schedulerJobRuns.jobId, jobId));
        await secondDb.delete(schedulerJobs).where(eq(schedulerJobs.id, jobId));
