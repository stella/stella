...
-- stella-migration-safety: reviewed delete-data - drops only the (country, court_pattern) keys the declaration above no longer carries, from an operator-seeded registry of 23 rows; rollback re-runs the previous release's seed
...
WHERE NOT EXISTS (
  ...
  ('HUN', 'alkotmánybíróság', 4, 'constitutional', 10),
  ('HUN', 'kúria|legfelsőbb bíróság', 3, 'supreme', 8),
  ('HUN', 'ítélőtábla', 2, 'appeal', 5),
  ('HUN', 'törvényszék|megyei bíróság|fővárosi bíróság', 2, 'regional', 4),
  ('HUN', 'közigazgatási és munkaügyi bíróság', 1, 'administrative-labour', 3),
  ('HUN', 'járásbíróság|kerületi bíróság|városi bíróság', 1, 'district', 2),
...
--> statement-breakpoint
-- stella-migration-safety: reviewed insert-select - the source relation is a 23-row VALUES list, not a table, so the statement is bounded and instant; rollback deletes the same (country, court_pattern) keys
