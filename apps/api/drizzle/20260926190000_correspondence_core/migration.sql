...
CREATE TABLE "correspondence" (
  ...
  "authenticated_sender_address" text NOT NULL,
  "original_signature" jsonb,
  "message_id" text,
  ...
  CONSTRAINT "correspondence_intake_check" CHECK ("intake" in ('direct', 'forwarded_inline', 'forwarded_attachment')),
  CONSTRAINT "correspondence_original_signature_check" CHECK ((("intake" = 'direct' and "original_signature" is null) or ("intake" in ('forwarded_inline', 'forwarded_attachment') and "original_signature" = '{"status":"unverified"}'::jsonb) or ("intake" = 'forwarded_attachment' and "original_signature"->>'status' = 'verified' and jsonb_typeof("original_signature"->'domain') = 'string' and length("original_signature"->>'domain') > 0)) is true),
...
CREATE TABLE "correspondence_allowed_senders" (
  ...
  CONSTRAINT "correspondence_allowed_senders_organization_fk" FOREIGN KEY ("organization_id") REFERENCES "organization"("id") ON DELETE cascade,
...
CREATE POLICY "correspondence_owner_offboarding_select" ON "correspondence" FOR SELECT TO public USING (current_user = (SELECT pg_catalog.pg_get_userbyid(relowner) FROM pg_catalog.pg_class WHERE oid = 'public.correspondence'::regclass) AND ("assignee_id" = nullif(current_setting('app.correspondence_offboarding_user_id', true), '') OR ("assignee_id" IS NULL AND "id" = ANY(nullif(current_setting('app.correspondence_offboarding_record_ids', true), '')::uuid[]))) AND (current_setting('app.correspondence_offboarding_scope', true) = 'account' OR (current_setting('app.correspondence_offboarding_scope', true) = 'organization' AND "organization_id" = current_setting('app.correspondence_offboarding_organization_id', true))));--> statement-breakpoint
CREATE POLICY "correspondence_owner_offboarding_update" ON "correspondence" FOR UPDATE TO public USING (current_user = (SELECT pg_catalog.pg_get_userbyid(relowner) FROM pg_catalog.pg_class WHERE oid = 'public.correspondence'::regclass) AND "assignee_id" = nullif(current_setting('app.correspondence_offboarding_user_id', true), '') AND (current_setting('app.correspondence_offboarding_scope', true) = 'account' OR (current_setting('app.correspondence_offboarding_scope', true) = 'organization' AND "organization_id" = current_setting('app.correspondence_offboarding_organization_id', true)))) WITH CHECK (current_user = (SELECT pg_catalog.pg_get_userbyid(relowner) FROM pg_catalog.pg_class WHERE oid = 'public.correspondence'::regclass) AND "assignee_id" IS NULL AND (current_setting('app.correspondence_offboarding_scope', true) = 'account' OR (current_setting('app.correspondence_offboarding_scope', true) = 'organization' AND "organization_id" = current_setting('app.correspondence_offboarding_organization_id', true))));--> statement-breakpoint
...
CREATE POLICY "matter_inbound_addresses_owner_lookup" ON "matter_inbound_addresses" FOR SELECT TO public USING (current_user = (SELECT pg_catalog.pg_get_userbyid(relowner) FROM pg_catalog.pg_class WHERE oid = 'public.matter_inbound_addresses'::regclass));--> statement-breakpoint
...
CREATE POLICY "correspondence_allowed_senders_owner_lookup" ON "correspondence_allowed_senders" FOR SELECT TO public USING (current_user = (SELECT pg_catalog.pg_get_userbyid(relowner) FROM pg_catalog.pg_class WHERE oid = 'public.correspondence_allowed_senders'::regclass));--> statement-breakpoint
...
CREATE POLICY "correspondence_allowed_sender_matters_owner_lookup" ON "correspondence_allowed_sender_matters" FOR SELECT TO public USING (current_user = (SELECT pg_catalog.pg_get_userbyid(relowner) FROM pg_catalog.pg_class WHERE oid = 'public.correspondence_allowed_sender_matters'::regclass));--> statement-breakpoint
