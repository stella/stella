...
-- ("ddd dd ddd") and no longer prefixes the name with "společnost", so its
-- text changed.
--
...
SET "format" = '**[company name]**, se sídlem [address], IČO: [registry number spaced], zapsaná v obchodním rejstříku vedeném [court instrumental] pod sp. zn. [file reference]'
WHERE "registry" = 'ares'
