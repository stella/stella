...
UPDATE "time_entries" AS entry
   SET "rate_at_entry" =
       ROUND(entry."rate_at_entry" * power(10::numeric, exponent.digits - 2))::bigint
  FROM (VALUES
  ('BHD', 3),
  ('BIF', 0),
  ('BYR', 0),
...
UPDATE "rate_entries" AS rate
   SET "hourly_rate" =
       ROUND(rate."hourly_rate" * power(10::numeric, exponent.digits - 2))::bigint
...
UPDATE "expenses" AS expense
   SET "amount" = GREATEST(
       1,
       ROUND(expense."amount" * power(10::numeric, exponent.digits - 2))::bigint
       )
  FROM (VALUES
  ('BHD', 3),
  ('BIF', 0),
