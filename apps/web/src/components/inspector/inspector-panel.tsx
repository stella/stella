...
export const InspectorPanel = ({ workspaceId }: InspectorPanelProps) => {
  ...
  const handleCloseTab = useCallback(
    (tabId: string) => {
      // oxlint-disable-next-line react/memo-dependencies -- docxActionsRef.current read at call time in a load-bearing callback (compiler-bailout component). react-compiler and react-hooks/exhaustive-deps disagree on this ref: exhaustive-deps requires it in the dep array (kept), the compiler flags it as unlistable — suppress the compiler side.
    ...
    <RenderStormRegion name="inspector">
      ...
      >
        ...
        {!minimized && activeTab?.type === "chat" && (
          ...
          // header, empty state with suggested skills, prompt-bar
...
type GenericInspectorViewProps = {
  tab: GenericTab;
  onClose: () => void;
};

/**
 * Render slot for a registry-backed view kind. Looks up the
 * registration for the tab's `type` and delegates rendering to it.
 * Falls back to `null` when no registration is found — that case
 * only fires while a route's lazy chunk (carrying the
 * `registerInspectorView` call) is still loading, or for stale tabs
 * synced in from a peer browser tab where the registering route
 * hasn't been visited yet.
 */
const GenericInspectorView = ({ tab, onClose }: GenericInspectorViewProps) => {
  const registration = getInspectorView(tab.viewType);
  if (registration === undefined) {
    return null;
  }
  // Mount as a React component, not a plain function call, so the
  // registered view owns its own React lifecycle — its hooks
  // (`useSuspenseQuery`, `useState`, …) subscribe to their own
  // re-render queue instead of being tied to this dispatcher's
  // closure. Without this, query invalidations in mutations
  // (install/remove) don't surface in the view body and stale
  // state lingers until a parent re-render kicks it.
