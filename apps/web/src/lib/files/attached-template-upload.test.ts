...
const waitForAttachedTemplatePrompt = async () => {
  for (let attempt = 0; attempt < 100; attempt += 1) {
    const prompt = currentAttachedTemplateUploadPrompt(
      useAttachedTemplateUploadStore.getState(),
    );
    if (prompt !== null) {
      return prompt;
    }
  ...
  throw new Error("Expected an attached-template upload prompt");
};

afterEach(() => {
  useAttachedTemplateUploadStore.setState({ prompts: [], nextPromptId: 1 });
});
...
describe("attached-template upload preparation", () => {
  ...
  test("does not rewrite a clean DOCX", async () => {
    const original = await makeDocxFile({
      relationships: `<Relationships xmlns="${RELATIONSHIPS_NAMESPACE}"><Relationship Id="rId2" Type="${HYPERLINK_TYPE}" Target="https://example.test" TargetMode="External"/></Relationships>`,
  ...
  test("refuses to rewrite an attached-template relationship without an ID", async () => {
    const original = await makeDocxFile({
      relationships:
        `<Relationships xmlns="${RELATIONSHIPS_NAMESPACE}">` +
        `<Relationship Type="${ATTACHED_TEMPLATE_TYPE}" ` +
  ...
  test("returns the sanitized replacement after approval", async () => {
    const original = await makeDocxFile({
      relationships:
        `<Relationships xmlns="${RELATIONSHIPS_NAMESPACE}">` +
        `<Relationship Id="rId1" Type="${ATTACHED_TEMPLATE_TYPE}" ` +
    ...
    const preflightPromise = preflightAttachedTemplateUpload([original]);
    const queued = await waitForAttachedTemplatePrompt();
    queued.prompt.onApproved();
    useAttachedTemplateUploadStore.getState().close(queued.id);
    const preflight = await preflightPromise;

    expect(preflight.type).toBe(ATTACHED_TEMPLATE_UPLOAD_PREFLIGHT.ready);
    if (preflight.type !== ATTACHED_TEMPLATE_UPLOAD_PREFLIGHT.ready) {
      throw new Error("Expected approved preflight to be ready");
    }
    const replacement = preflight.replacements.get(original);
    expect(replacement).toBeInstanceOf(File);
    expect(replacement).not.toBe(original);
  });

  test("returns cancellation without exposing replacements", async () => {
    const original = await makeDocxFile({
      relationships:
        `<Relationships xmlns="${RELATIONSHIPS_NAMESPACE}">` +
        `<Relationship Id="rId1" Type="${ATTACHED_TEMPLATE_TYPE}" ` +
        'Target="https://example.test/Contract.dotm"/>' +
        "</Relationships>",
      settings:
        '<w:settings xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main" ' +
        'xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">' +
    ...
    const preflightPromise = preflightAttachedTemplateUpload([original]);
    const queued = await waitForAttachedTemplatePrompt();
    queued.prompt.onCancelled();
    useAttachedTemplateUploadStore.getState().close(queued.id);

    expect(await preflightPromise).toEqual({
      type: ATTACHED_TEMPLATE_UPLOAD_PREFLIGHT.cancelled,
    });
  });
});
