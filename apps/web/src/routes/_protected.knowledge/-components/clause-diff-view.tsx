  ...
  <div className="space-y-1">
    {diffs.map((para, i) => (
      <p
        ...
        // oxlint-disable-next-line react/no-array-index-key -- diffs is a read-only paragraph diff recomputed fresh on every render (whole-list replace); paragraphs are non-interactive with no per-item state.
        key={i}
      >
        {para.segments.map((seg, j) => (
          <span
            ...
            // oxlint-disable-next-line react/no-array-index-key -- segments is a read-only word-diff recomputed fresh on every render (whole-list replace); spans are non-interactive with no per-item state.
            key={j}
          >
            {seg.text}
          </span>
