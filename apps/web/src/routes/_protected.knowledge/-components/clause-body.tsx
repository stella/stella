...
const ClauseListItems = ({ items }: { items: ListTreeItem[] }) => (
  <>
    {items.map((item, idx) => (
      // oxlint-disable-next-line react/no-array-index-key -- read-only clause list content parsed from source on every render (whole-list replace); items are non-interactive with no per-item state.
    ...
    return (
      <>
        {paragraph.runs.map((run, ri) => {
          ...
          // oxlint-disable-next-line react/no-array-index-key -- read-only clause paragraph runs parsed from source on every render (whole-list replace); spans are non-interactive with no per-item state.
