...
export const AiSuggestFields = ({ getView }: AiSuggestFieldsProps) => {
  ...
  const handleSuggest = async () => {
    ...
    setProposals(next);
    if (next.length === 0) {
      stellaToast.add({ type: "info", title: t("templates.studio.aiNoFields") });
    }
  };
  ...
  return (
    <Popover onOpenChange={handleOpenChange} open={open}>
      ...
      />
      <PopoverPopup align="end" className="flex w-96 flex-col gap-3 p-3">
        ...
        <Button disabled={loading} onClick={() => void handleSuggest()} size="sm">
          {loading ? (
