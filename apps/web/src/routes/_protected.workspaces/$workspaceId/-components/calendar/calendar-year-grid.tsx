        ...
        <span className="absolute bottom-0 flex gap-px">
          {dots.slice(0, MAX_DOTS).map((dot, i) => (
            <span
              ...
              // oxlint-disable-next-line react/no-array-index-key -- YearDot is a decorative date+color pair with no entity id (multiple entities can share a color); dots are non-interactive and capped at MAX_DOTS with no per-item state.
              key={`${dot.color}-${i}`}
