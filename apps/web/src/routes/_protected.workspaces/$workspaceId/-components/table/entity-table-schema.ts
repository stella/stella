/**
 * The columns a matter's table view has, in order, and what each of them can
 * do. What a header or a cell draws lives in `table-columns.tsx`; this module
 * carries only the description, which is what makes the column set testable
 * without rendering a table.
 *
 * The column-render union and the utility columns are shared with every other
 * row kind (`@/components/workspaces/table/table-schema`); only the entity
 * half's column list is here, because it reads a saved view.
 */
