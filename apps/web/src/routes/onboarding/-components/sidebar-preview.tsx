...
export const SidebarPreview = ({
  ...
  <div className="bg-background w-[340px] overflow-hidden rounded-xl shadow-lg">
    ...
    <div className="border-b px-4 py-2.5">
      <span className="text-foreground text-sm font-medium">
        {organizationName ? (
          ...
          <span className="text-muted-foreground/50">Your team</span>
        )}
      </span>
    </div>
    ...
    <div className="flex flex-col gap-0.5 px-3 py-2">
      <NavItem active icon={LayersIcon} label="Matters" />
      <NavItem icon={BookOpenIcon} label="Knowledge" />
      <NavItem icon={MessageCircleIcon} label="Chat" />
    </div>
    ...
    <div className="px-3 py-2">
      ...
      {dmsCount > 0 ? (
        ...
        <div className="animate-slide-in flex items-center gap-2 rounded-md px-2 py-1.5">
          ...
          <span className="text-foreground truncate text-sm">{matterName}</span>
        </div>
      ) : (
