...
describe("browser command identity", () => {
  ...
  test("a navigation after the user moved the page on is stale", () => {
    ...
    for (const command of [
      ...
      for (const live of [
        ...
        ).toEqual({ status: "page-unconfirmed" });
      }
    }
  });
  ...
  test("an unknown page identity refuses the navigation", () => {
    ...
    for (const navigation of [
      ...
      {
        live: { documentId: "document-top", url: URL_A },
        settled: null,
      },
    ]) {
      expect(
  ...
  test("an error page is no exception", () => {
    expect(
      checkCommandIdentity({
        command: { action: "open", url: URL_A },
        controlledTab,
        navigation: {
          live: { documentId: "document-error", url: "https://printer.local/" },
