...
export const aiOverview = [
  ...
  "stella is open source under Apache-2.0 and can run on your own infrastructure or on the managed cloud. It uses standard formats with full export, so there is no lock-in, and self-hosted deployments carry no seat licences. AI outputs are grounded by citations traceable back to the source text, and sensitive material can be anonymized before AI workflows.",
] as const;
