...
const illustrations = [ReviewIllustration, DraftIllustration, ResearchIllustration];
    ...
    <div className="w-full">
      <div className="showcase-grid flex snap-x snap-mandatory gap-4 overflow-x-auto pb-1 md:grid md:grid-cols-3 md:items-start md:overflow-visible md:pb-0">
        {primaryCapabilities.map((item, index) => {
          ...
          return (
            <article
              className="showcase-tile snap-center self-start min-w-[82%] transition-[opacity,transform] duration-300 ease-out sm:min-w-[20rem] md:min-w-0"
              key={item.title}
            >
              <div
                className={cn(
                  "group relative flex aspect-[0.75] min-h-[20rem] items-center justify-center overflow-hidden rounded-[1.45rem] border border-border/45 bg-[color-mix(in_srgb,var(--background)_90%,var(--auth-gradient-end)_10%)] shadow-[0_14px_34px_-28px_rgba(0,0,0,0.16)] transition-[border-color,box-shadow,transform] duration-300 ease-out sm:min-h-[24rem]",
                )}
                data-showcase-shell
              >
...
function ReviewIllustration() {
  return (
    <svg viewBox="0 0 320 280" fill="none" className="h-full w-full">
      {/* Table header */}
      <rect x="40" y="40" width="240" height="28" rx="6" fill="currentColor" opacity="0.06" />
      <rect x="52" y="50" width="48" height="8" rx="3" fill="currentColor" opacity="0.15" />
      <rect x="120" y="50" width="32" height="8" rx="3" fill="currentColor" opacity="0.1" />
      <rect x="172" y="50" width="28" height="8" rx="3" fill="currentColor" opacity="0.1" />
      <rect x="220" y="50" width="44" height="8" rx="3" fill="currentColor" opacity="0.1" />
      {/* Rows */}
      {[0, 1, 2, 3, 4].map((i) => {
        const y = 80 + i * 36;
        ...
        return (
          <g key={i}>
            <rect x="40" y={y} width="240" height="28" rx="6"
              fill={active ? "#549CD1" : "currentColor"}
              opacity={active ? 0.08 : 0.03}
            />
            <rect x="52" y={y + 10} width={36 + (i * 7) % 20} height="8" rx="3"
              fill="currentColor" opacity={active ? 0.2 : 0.1}
            />
            <rect x="120" y={y + 10} width="24" height="8" rx="3"
              fill="currentColor" opacity={0.08}
            />
            <circle cx="184" cy={y + 14} r="4"
              fill={active ? "#549CD1" : "currentColor"}
              opacity={active ? 0.5 : 0.1}
            />
            <rect x="220" y={y + 10} width={20 + (i * 11) % 24} height="8" rx="3"
              fill="currentColor" opacity={0.08}
            />
          </g>
        );
...
function DraftIllustration() {
  return (
    <svg viewBox="0 0 320 280" fill="none" className="h-full w-full">
      <rect x="60" y="30" width="200" height="220" rx="10" fill="currentColor" opacity="0.03"
        stroke="currentColor" strokeOpacity="0.08" strokeWidth="1"
      />
      <rect x="80" y="50" width="100" height="10" rx="4" fill="currentColor" opacity="0.15" />
      <line x1="80" y1="72" x2="240" y2="72" stroke="currentColor" strokeOpacity="0.06" />
      <rect x="80" y="84" width="40" height="6" rx="2" fill="currentColor" opacity="0.1" />
      <rect x="80" y="96" width="140" height="20" rx="5" fill="#549CD1" opacity="0.06"
        stroke="#549CD1" strokeOpacity="0.15" strokeWidth="1"
      />
      <rect x="90" y="103" width="60" height="6" rx="2" fill="#549CD1" opacity="0.2" />
      <rect x="80" y="128" width="36" height="6" rx="2" fill="currentColor" opacity="0.1" />
      <rect x="80" y="140" width="140" height="20" rx="5" fill="currentColor" opacity="0.03"
        stroke="currentColor" strokeOpacity="0.08" strokeWidth="1"
      />
      <rect x="90" y="147" width="80" height="6" rx="2" fill="currentColor" opacity="0.1" />
      <rect x="80" y="176" width="150" height="6" rx="2" fill="currentColor" opacity="0.08" />
      <rect x="80" y="188" width="130" height="6" rx="2" fill="currentColor" opacity="0.06" />
      <rect x="80" y="200" width="144" height="6" rx="2" fill="currentColor" opacity="0.06" />
      <rect x="80" y="212" width="90" height="6" rx="2" fill="currentColor" opacity="0.05" />
      <rect x="80" y="176" width="1.5" height="14" rx="0.5" fill="#549CD1" opacity="0.4" />
    </svg>
  );
}
    ...
    <svg viewBox="0 0 320 280" fill="none" className="h-full w-full">
      ...
      {nodes.map((n, i) => (
        <g key={`n${i}`}>
          ...
          />
          <rect x={n.x + n.r + 6} y={n.y - 3} width={n.primary ? 32 : 20} height="6" rx="2"
            fill="currentColor" opacity={n.primary ? 0.12 : 0.06}
          />
        </g>
      ))}
    </svg>
