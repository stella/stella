...
| `templates.discover`              | read               | stella:templates                         | —       | not runnable over the generic transport: requires a file in `file`, which JSON cannot carry. No JSON-transport alternative: marker discovery reads the supplied DOCX; for a template already stored, templates.get returns the discovered fields                      |
