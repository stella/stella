...
| `clipboard-write` — Writing text to the system clipboard from the browser | `apps/web/src/lib/copy-to-clipboard.ts` | global `navigator.clipboard` | `navigator.clipboard.writeText` rejects on a denied permission or an insecure context, and every call site owes the user that outcome. The owner wraps it in a `Result`, so callers branch on the failure instead of each growing its own try/catch. |
