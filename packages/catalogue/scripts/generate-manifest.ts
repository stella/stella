...
const entries = CATALOGUE_KINDS.flatMap((kind) => {
  ...
  return readdirSync(kindDir, { withFileTypes: true })
    ...
    .map((slug): GeneratedEntry | null => {
      ...
      let resourceFiles: GeneratedResourceFile[] = [];
      if (kind === "skill") {
        ...
        resourceFiles = (manifestRaw.resources ?? []).map((resourcePath) => {
...
const entryRows = entries.map(
  (entry) =>
    `  { manifest: ${entry.importName}, icon: ${entry.iconLiteral}, body: ${entry.bodyLiteral ?? "null"}, resourceFiles: ${entry.resourceFilesLiteral} }`,
);
