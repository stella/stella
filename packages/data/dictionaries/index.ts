 ...
 *
 * City dictionaries and `loadDictionaryBundle` live in
 * the sibling `./cities` entry. This module must never
 * reach `./city-loaders`, statically or transitively:
 * that map holds one literal `import()` per country, so
 * touching it makes a bundler emit all ~237 city chunks
 * for every consumer of this entry.
 */
...
export const DICTIONARY_META = {
  ...
  // loadCityDictionary() from ./cities — not registered
  // here. See generate-cities.ts for the GeoNames
  // pipeline and city-loaders.ts for the loader map.
...
const LOADERS: Record<DictionaryId, () => Promise<JsonModule>> = {
  ...
  // ── Cities: loaded via loadCityDictionary() in ./cities
