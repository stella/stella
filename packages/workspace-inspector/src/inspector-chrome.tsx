 ...
 *
 * The class strings are Stella's workspace inspector verbatim
 * (apps/web/src/components/inspector/*, apps/web/src/components/workspaces/
 * matter-metadata-sheet.tsx) so a consuming app renders the same component
 * family rather than something merely similar.
 *
 * Deliberately headless where a library would otherwise leak: the rail and
 * the tab strip take rendered children, so the host picks its own tabs
 * primitive without this package pinning one.
 */
...
export const InspectorDescription = ({
  ...
  <p
    className={cn("text-muted-foreground truncate text-[11px]", className)}
